function geneticalgo_ex3_griewangk_i3
%2012-58922
%Cmsc 191
%Genetic Algorithm
%Exercise 3 (2 of 4)
...
%           f(a,b) =  1 + ((x(1)^2)+(x(2)^2)+(x(3)^3))/4000 - (cos(x(1)/sqrt(1))*cos(x(2)/sqrt(2))*cos(x(3)/sqrt(3)));
%where a,b,c is in [-600,600]
...
tpl = 3;        %i = 2 i.e. 2-Tuple Griewangk Function &and for ease of debugging
...
%Tf = table(pnum',Chromosome(:,1),Chromosome(:,2),Chromosome(:,3),F_obj(:),'VariableNames',{'Number','a','b','c','Fitness'})
%disp(Tf)
...
Tsol = table(Chromosome(cn,1),Chromosome(cn,2),Chromosome(cn,3),fittest,'VariableNames',{'a','b','c','Fitness'});
disp(Tsol)
...
function F_obj_ret = OBJFUNC_GRIEWANGK(x)
    F_obj_ret = abs(1 + ((x(1)^2)+(x(2)^2)+(x(3)^3))/4000 - (cos(x(1)/sqrt(1))*cos(x(2)/sqrt(2))*cos(x(3)/sqrt(3))));
    %Objective: The Fitness Function must be very close if not equal to 0
end

function Chromosome_ret = CROSSOVER(PChromosome1,PChromosome2,cp_rec)
    Chromosome_ret = PChromosome1;
    for i=cp_rec+1:3    %crossovers after the crossover point
