function geneticalgo_ex3_sumdifpow_i3
%%
...
%Exercise 3 (4 of 4)
%%
%SUMDIFPOW'S FUNCTION
%The 2-tuple Sum of Different Powers Function is:
%           f(x) = abs(a)^2 + abs(b)^3 + abs(c)^4
%such that a,b is in [-1,1]
...
pnum = zeros(1);
for i=1:popsize
    Chromosome(i,:) = 2*(rand(1,tpl)-0.5);  %initializing first generation
    pnum(i) = i;
end
%%
...
F_obj = zeros(1);
for i=1:popsize
   F_obj(i) = OBJFUNC_SUMDIFPOW(Chromosome(i,:));
end
%%
%STEP 3: Selection
Total = 0;
...
for i=1:mu
    ...
    rmn = 2*(rand(1)-0.5);     %random mutation number
...
function F_obj_ret = OBJFUNC_SUMDIFPOW(x)
    F_obj_ret = abs(x(1))^2 + abs(x(2))^3 + abs(x(3))^4;
    %Objective: The Fitness Function must be very close if not equal to 0
end
